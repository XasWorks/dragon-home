...
module XNM
    module DragonHome
        class Cookie
            ...
            def speak_text(text, blocking: true, volume: 0.7)
                if(text.is_a? String)
                    tts_cfg = { text: text } 
                else
                    tts_cfg = text.clone
                ...
                File.delete(tts_cfg[:ttsfile]) if File.exists? tts_cfg[:ttsfile]
                @current_audio_source.volume *= volume
            ...
            def play_file(file, blocking: true, volume: 1)
